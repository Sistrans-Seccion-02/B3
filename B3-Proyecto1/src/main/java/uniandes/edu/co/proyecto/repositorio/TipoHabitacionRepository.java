...
public interface TipoHabitacionRepository extends JpaRepository<TipoHabitacion, Long> {
    ...
    void insertarTipoHabitacion(@Param("nombre") String nombre, @Param("dotacion") String dotacion, @Param("capacidad") int capacidad);

    @Modifying
    @Transactional
    @Query(value = "UPDATE tipo_habitacion SET nombre=:nombre, dotacion=:dotacion, capacidad=:capacidad WHERE id=:id", nativeQuery = true)
    void actualizarTipoHabitacion(@Param("id") int id, @Param("nombre") String nombre, @Param("dotacion") String dotacion, @Param("capacidad") int capacidad);
